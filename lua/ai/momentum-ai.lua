...
wendao_skill.getTurnUseCard = function(self)
	if not self.player:hasUsed("WendaoCard") then
		...
		if invoke ~= "no" then
			if invoke == "eq" then
				assert(owner)
				if owner:hasArmorEffect("PeaceSpell", false) then
...
sgs.ai_slash_prohibit.PeaceSpell = function(self, from, enemy, card)
	...
	if self:hasArmorEffect(enemy, "PeaceSpell") and card:isKindOf("NatureSlash") and not IgnoreArmor(from, enemy) and not from:hasWeapon("IceSword") then return true end
	return
end
...
function sgs.ai_armor_value.PeaceSpell(player, self)
	...
	if getCardsNum("Peach", player, player) + getCardsNum("Analeptic", player, player) == 0 and player:getHp() == 1 then
		if player:hasArmorEffect("PeaceSpell", false) then return 99
		else return -99
		end
	end
	return 3.5
